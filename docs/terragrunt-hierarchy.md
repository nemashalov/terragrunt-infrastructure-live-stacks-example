...
- **Stack (`terragrunt.stack.hcl`)**: The entrypoint for an application slice. A stack describes *which* reusable building blocks to deploy, *how* they depend on each other, and *what configuration* each one should receive. It can point at several catalog modules (service, DB, security groups, etc.), set shared locals, and express dependency ordering. When you run `terragrunt run-all apply` (or `terragrunt apply` if you generate unit configs) from a stack directory, Terragrunt walks the stack definition and applies every referenced unit with the values defined here, guaranteeing the whole slice comes up consistently.
